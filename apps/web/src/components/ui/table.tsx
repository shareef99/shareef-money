...
export interface TableProps<THeader extends TableHeader> {
  header: THeader;
  rows: RowFromHeader<THeader>[];
  withColors?: boolean;
  emptyStateText?: string;
  enableColumnVisibility?: boolean;
  renderActions?: (row: RowFromHeader<THeader>) => React.ReactNode;
}
...
export function Table<THeader extends TableHeader>({
  ...
  const [sortKey, setSortKey] = useState<keyof THeader | null>(
    headerKeys[0] ?? null,
  );
  ...
  return (
    <div className="overflow-x-auto rounded-xl border border-border bg-card shadow-sm">
      ...
      >
        <MantineTable.Thead
          className={twMerge(
            withColors ? "bg-blue-50/60 dark:bg-blue-950/30" : "bg-muted/40",
          )}
        >
          <MantineTable.Tr>
            {visibleKeys.map((key) => {
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <Tooltip
                      label={
                        isCurrent ? `Sorted ${sortDirection}` : "Sort column"
                      }
                    >
                      <ActionIcon
                        variant="muted"
                        color={withColors && isCurrent ? "blue" : "gray"}
                        size="sm"
            ...
            {renderActions && (
              <MantineTable.Th className="whitespace-nowrap text-foreground text-right">
                Actions
              </MantineTable.Th>
            )}
          </MantineTable.Tr>
