  ...
  return (
    <nav className="flex flex-col h-full py-4 px-3">
      <div className="mb-8 px-2">
        <h2 className="text-lg font-bold text-text">Shareef Money</h2>
      </div>
      ...
      <div className="flex flex-col gap-1 flex-1">
        {navItems.map(({ to, label, icon: Icon }) => (
          <Link
            ...
            to={to}
            onClick={onNavigate}
            className="flex items-center gap-3 px-3 py-2.5 rounded-md text-sm font-medium text-text-secondary hover:bg-card transition-colors"
            activeProps={{
              className:
